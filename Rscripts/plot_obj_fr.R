...
tradewide$variable <- revalue(tradewide$variable, replace = c("y3" = "3 ans",
                                                "y5" = "5 ans",
                                                "y10" = "10 ans"))


IE_names_fr <- c(OMbase="MEbase",
                 OMcore1="MEcore1",
                 OMcore2="MEcore2",
                 OMcore3="MEcore3",
                 OMcore4="MEcore4",
...
ptrade1 <- ggplot(tradewide,aes(x=obj3,y=obj1,col=factor(MP),shape=variable))+
    geom_hline(yintercept=0.75,linetype='dashed',col='darkgreen')+
    geom_point()+
    facet_wrap(~OM,ncol=4,labeller=as_labeller(IE_names_fr))+
    scale_color_viridis_d()+
    labs(x='Prises (t)',y='Probabilité BSR>PRL',col='RCP',shape='Temps')+
    theme(strip.background = element_blank())
saveplot(ptrade1,name="Trade_obj1",dim=c(15,11),wd='img/resdoc/fr')
...
ptrade2 <- ggplot(tradewide,aes(x=obj3,y=obj2,col=factor(MP),shape=variable))+
    ...
    facet_wrap(~OM,ncol=4,labeller=as_labeller(IE_names_fr))+
    scale_color_viridis_d()+
    labs(x='Prises (t)',y='Probabilité de déclin',col='RCP',shape='Temps')+
    theme(strip.background = element_blank())+
    scale_y_continuous(expand = c(0,0),limits = c(0,1))
saveplot(ptrade2,name="Trade_obj2",dim=c(15,11),wd='img/resdoc/fr')
...
for(i in t){
    ptrade1sub <- ggplot(tradewide[tradewide$variable==i,],aes(x=obj3,y=obj1,col=factor(MP)))+
        ...
        facet_wrap(~OM,ncol=4,labeller=as_labeller(IE_names_fr))+
        scale_color_viridis_d()+
        labs(x='Prises (t)',y='Probabilité BSR>PRL',col='RCP',title = i)+
        theme(strip.background = element_blank())
    saveplot(ptrade1sub,name=paste0("Trade_obj1_",tshort[which(i==t)]),dim=c(18,10),wd='img/resdoc/fr')
    ptrade2sub <- ggplot(tradewide[tradewide$variable==i,],aes(x=obj3,y=obj2,col=factor(MP)))+
        geom_rect(aes(ymin=0.5,ymax=1,xmin=-Inf,xmax=Inf),fill='lightgrey',col='lightgrey',alpha=0.5)+
        geom_point()+
        facet_wrap(~OM,ncol=4,labeller=as_labeller(IE_names_fr))+
        scale_color_viridis_d()+
        labs(x='Prises (t)',y='Probabilité de déclin',col='RCP',title = i)+
        theme(strip.background = element_blank())+
        scale_y_continuous(expand = c(0,0),limits = c(0,1))
    saveplot(ptrade2sub,name=paste0("Trade_obj2_",tshort[which(i==t)]),dim=c(18,10),wd='img/resdoc/fr')
}
